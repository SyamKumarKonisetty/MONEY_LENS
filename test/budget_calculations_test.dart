...
void main() {
  test('Budget calculations validation: set and update monthly budget', () async {
    ...
    expect(container.read(budgetNotifierProvider), const AsyncValue<List<BudgetEntity>>.loading());

    // Watch the notifier to kickstart listening
    final subscription = container.listen(budgetNotifierProvider, (previous, next) {});

    // Set budget amount to ₹45,000 for Food
    await container.read(budgetNotifierProvider.notifier).setBudget('food', 45000.0);
    ...
    await container.read(budgetNotifierProvider.notifier).setBudget('food', 60000.0);
    await Future<void>.delayed(Duration.zero);
